...
SYSTEM_PROMPT_VQA = '''
请描述你看到的物体，要简洁凝练，保持口语化
我现在的指令是：
'''

'''
以及说出潜在物体之间的联系（比如，水放在电子设备旁边三危险的）

例如：
我观察到桌面上有一个智能手机和一个没盖盖子的水杯，可千万别把水撒到手机上了哦
...
def post_processing_viz(result, img_path, check=False):
    ...
    if start_depth is not None:
        ...
        draw.text((START_X_MIN, START_Y_MAX - 25), start_depth_text, font=font, fill=(255, 0, 0, 1))

    if end_depth is not None:
        end_depth_text = f"Depth: {end_depth:.2f} mm"
        draw.text((END_X_MIN, END_Y_MAX - 25), end_depth_text, font=font, fill=(0, 0, 255, 1))
