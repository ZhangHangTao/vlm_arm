...
def calculate_arm_height(camera_depth, camera_depth_max=313, arm_z_max=119.0):
    """
    将相机的深度信息转换为机械臂的 Z 坐标。
    :param camera_depth: 相机检测的深度值
    :param camera_depth_max: 相机检测的最大深度
    :param arm_z_max: 机械臂的最xiao Z 坐标
    :return: 机械臂的 Z 坐标
    """
    camera_depth=min(camera_depth,camera_depth_max+100)
    return arm_z_max+(camera_depth_max-camera_depth)
...
def vlm_move(PROMPT='帮我把绿色方块放在小猪佩奇上', input_way='keyboard'):
    ...
    while n < 5:
        ...
        try:
            ...
            if START_X_CENTER == -1 or not (20 <= START_Z_CAMERA <= 1000 and 20 <= END_Z_CAMERA <= 1000):
